...
@Configuration("TaxModule")
@EnableTransactionManagement
public class TaxModule implements Module {
    ...
    private static Logger log = Logger.getLogger(TaxModule.class);
    ...
    @Bean
    @Scope(value = "singleton")
    @Qualifier("taxDataSource")
    BasicDataSource dataSource() {
        if (dataSource == null) {
            h2Server();
            TypedProperties properties = new TypedProperties();
            properties.put(DB_POOL_DRIVER, env.getProperty(DB_POOL_DRIVER, Driver.class.getName()));
            properties.put(DB_POOL_URL, env.getProperty(DB_POOL_URL, "jdbc:h2:mem:tax"));
